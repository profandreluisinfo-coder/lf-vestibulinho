...
class ExportController extends Controller
{
    public function exportUsers()
    {
        ...
        if (!Inscription::exists()) {
            return redirect()->route('admin.painel')->with('error', 'Nenhuma inscrição encontrada.');
        }

        // Verificar se existe prova agendada
        if (!ExamResult::exists()) {
            return redirect()->route('admin.painel')->with('error', 'Não é possível exportar os candidatos, pois não existe uma prova agendada.');
        }
