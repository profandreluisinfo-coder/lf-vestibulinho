...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @keyframes pulse     { 0%,100%{box-shadow:0 0 0 0 rgba(39,174,96,.35)} 50%{box-shadow:0 0 0 10px rgba(39,174,96,0)} }
    ...
    .deco-c3 { width:160px;height:160px; top:40%; right:8%;
               border:1px solid rgba(39,174,96,.25); animation:float 7s ease-in-out infinite; }
    .deco-blob {
    ...
    .panel-center .headline em { color:#6eefb0; font-style:normal; }
    ...
    .tips-card {
      margin-top:2.2rem; background:rgba(255,255,255,.07);
      border:1px solid rgba(255,255,255,.12); border-radius:var(--radius);
      padding:1.2rem 1.4rem; backdrop-filter:blur(6px);
      animation:fadeUp .8s ease .5s both;
    }
    .tips-card h6 {
      font-size:.75rem; font-weight:700; color:#6eefb0;
    ...
    .tip-icon {
      ...
      background:rgba(39,174,96,.2); color:#6eefb0;
      display:flex; align-items:center; justify-content:center; font-size:.65rem;
    }
    .tip-item p { font-size:.78rem; color:rgba(255,255,255,.6); line-height:1.55; margin:0; }

    /* Progresso de etapas */
    .step-progress {
      display:flex; align-items:center; gap:.5rem;
      margin-top:2rem; animation:fadeUp .7s ease .9s both;
    }
    .sp-step {
      display:flex; align-items:center; gap:.5rem;
    ...
    .sp-dot.done    { background:rgba(0,168,150,.4); color:var(--teal); border:1.5px solid var(--teal); }
    .sp-dot.current { background:rgba(39,174,96,.25); color:#6eefb0; border:1.5px solid #6eefb0; }
    .sp-line { flex:1; height:1px; background:rgba(255,255,255,.1); max-width:24px; }
    ...
    .form-badge {
      ...
      color:var(--green2); border-radius:50px;
    ...
    .toggle-pwd-btn {
      ...
      font-family:var(--font-head); cursor:pointer; border-radius:50px;
      padding:.3rem .9rem; display:inline-flex; align-items:center; gap:.4rem;
      transition:background .2s, box-shadow .2s;
    }
    .toggle-pwd-btn:hover { background:rgba(0,168,150,.15); box-shadow:0 2px 10px rgba(0,168,150,.2); }
    ...
    .field-label .req { color:#e74c3c; font-size:.7rem; }
    ...
    .form-input.input-ok ~ .field-icon { color:var(--green); }
    ...
    .eye-btn {
      ...
      font-size:.95rem; cursor:pointer; padding:.2rem; line-height:1;
      transition:color .2s;
    }
    ...
    /* Barra de força */
    .strength-wrap { margin-top:.55rem; }
    .strength-track {
      height:5px; background:rgba(11,30,61,.08); border-radius:3px; overflow:hidden;
    }
    .strength-fill {
    ...
    .strength-label {
      ...
      display:block; margin-top:.3rem; min-height:1rem; transition:color .3s;
    }

    /* Regras checklist */
    .rules-box {
      background:rgba(11,30,61,.03); border-radius:12px;
      border:1px solid rgba(11,30,61,.07); padding:.85rem 1rem;
    }
    .rules-note {
      font-size:.78rem; color:var(--muted); line-height:1.65;
      border-left:3px solid var(--amber); padding-left:.75rem; margin-bottom:.85rem;
    }
    .rules-grid { display:grid; grid-template-columns:1fr 1fr; gap:.35rem .6rem; }
    .rule-chip {
      display:flex; align-items:center; gap:.4rem;
      font-size:.77rem; color:var(--muted); font-weight:500; transition:color .3s;
    }
    .rule-chip .ri {
    ...
    .rule-chip.valid { color:var(--green); }
    .rule-chip.valid .ri {
      background:var(--green); border-color:var(--green); color:#fff;
      transform:scale(1.15); animation:checkPop .3s cubic-bezier(.22,.68,0,1.3);
    }
    ...
    .btn-reset:disabled {
      opacity:.42; cursor:not-allowed; box-shadow:none; transform:none !important;
    }
    .btn-reset:not(:disabled):hover {
      transform:translateY(-3px); box-shadow:0 12px 34px rgba(39,174,96,.45);
    }

    /* Link inferior */
    .form-links {
      border-top:1px solid rgba(11,30,61,.07);
      margin-top:1.3rem; padding-top:1.1rem; text-align:center;
    }
    .link-back {
    ...
    .link-back:hover { color:var(--navy); gap:.65rem; }
    ...
    .success-card { text-align:center; animation:scaleIn .55s cubic-bezier(.22,.68,0,1.2) both; }
    .success-ring {
    ...
    .success-card p  { font-size:.87rem; color:var(--muted); line-height:1.7; max-width:280px; margin:0 auto; }
    ...
    .btn-go-login {
      ...
      transition:transform .25s, box-shadow .25s; margin-top:1.5rem;
    }
    .btn-go-login:hover {
      transform:translateY(-3px); box-shadow:0 12px 30px rgba(0,168,150,.45); color:#fff;
    }
...
<div class="page-wrapper">
    ...
    <div class="panel-center">
      ...
      <div class="tips-card">
        ...
        <div class="tip-item">
          ...
          <p>Misture letras <strong style="color:#fff;">maiúsculas</strong> e <strong style="color:#fff;">minúsculas</strong> para aumentar a complexidade.</p>
        </div>
        <div class="tip-item">
          <div class="tip-icon"><i class="bi bi-check-lg"></i></div>
        ...
        <div class="tip-item">
          ...
          <p>Evite sequências óbvias como <strong style="color:#f87171;">123456</strong> ou seu próprio nome.</p>
        </div>
      </div>
  ...
  <main class="panel-right">
    <div class="form-card">
      ...
      <!-- ─── VIEW 1: Formulário ─────────────────────────── -->
      {{-- <div class="view @if(session('success')) hidden @endif" id="viewForm"> --}}<div class="view" id="viewForm">
        ...
        <form method="POST" action="{{ route('reset.password.action') }}" id="resetForm">
          ...
          {{-- @if ($errors->any())
            <div class="alert alert-danger py-2 px-3 mb-3 rounded-3" style="font-size:.82rem;">
              <ul class="mb-0 ps-3">
          ...
          <div>
            ...
            <div class="field-wrap">
              <input type="password" id="newPwd" name="password" class="form-input pwd-input @error('password') input-error @enderror"
                placeholder="Crie sua nova senha"
                style="padding-right:2.8rem;"
                oninput="onPwdInput()" />
              <i class="bi bi-lock-fill field-icon"></i>
              <button class="eye-btn" type="button"
                onclick="toggleEye('newPwd','eye1')" aria-label="Mostrar senha">
          ...
          <div>
            ...
            <div class="field-wrap">
              <input type="password" id="confirmPwd" name="password_confirmation" class="form-input pwd-input @error('password_confirmation') input-error @enderror"
                placeholder="Repita a nova senha"
                style="padding-right:2.8rem;"
                oninput="validateConfirm()" />
              <i class="bi bi-shield-lock-fill field-icon"></i>
              <button class="eye-btn" type="button"
                onclick="toggleEye('confirmPwd','eye2')" aria-label="Mostrar senha">
          ...
          <!-- Botão -->
          <button class="btn-reset" id="btnSubmit" type="submit"
            disabled>
...
<script>
  ...
  function onPwdInput() {
    ...
    const map = [
      ...
      { w:'30%',  bg:'#e74c3c',     txt:'Muito fraca'  },
      { w:'55%',  bg:var_amber(),   txt:'Fraca'         },
      { w:'78%',  bg:'#f0c030',     txt:'Razoável'      },
      { w:'100%', bg:var_green(),   txt:'Forte ✓'       },
    ];
  ...
  function validateConfirm() {
    ...
    if (!v) {
      msg.innerHTML = ''; msg.className = 'field-msg'; return;
    }
    if (v !== pwd) {
  ...
  //     style="width:.9rem;height:.9rem;border-width:2px;"></span> Salvando...`;
  //   btn.disabled = true;
  //   btn.style.opacity = '1';
  //   // O form prossegue normalmente com o submit real
  // });
